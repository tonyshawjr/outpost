<script>
  ...
  let { editor, oncontext, preview = false } = $props();
        ...
        {#each ALL as d (d.key)}
          <div class="device" style:width={`${d.w}px`}>
            ...
            <div class="device-frame">
              <CanvasFrame {editor} fitHeight viewportHeight={d.h} onwheel={applyWheel} {preview} />
            </div>
          </div>
        {/each}
      </div>
  ...
  {:else}
    <div class="stage" class:fluid={mode === 'desktop'}>
      <div class="single" style:width={SINGLE_WIDTH[mode] ? `${SINGLE_WIDTH[mode]}px` : '100%'}>
        <CanvasFrame {editor} oncontext={preview ? undefined : oncontext} {preview} />
      </div>
    </div>
  {/if}
</div>
