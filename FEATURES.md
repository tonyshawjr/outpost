...
- **Click-target picking (`#14`, v6.0.0-beta.42).** A click interaction can drive *another* element: "Pick element…" enters a canvas pick-mode (`outpost:motion-pick` → `motionPickFor` in NodeBuilder; `CanvasFrame` routes the pick click to `onpick` via a `livePick` ref so toggling doesn't remount the iframe; crosshair cursor + status banner). The source stores `motion.target`; the bake emits `tg:<id>` on the source and `data-mid="<id>"` on the target (both gated `^n_[a-z0-9]+$`, target set collected per-render), and the runtime reveals `[data-mid]` on source click. Verified live: source `tg` + target `data-mid` + runtime all present.
- **Sequence timeline (`#13`, v6.0.0-beta.43).** `MotionTimeline.svelte` — a bottom panel (toggled from the toolbar) that lists every non-scroll motion node as a track on a time axis. Drag a bar (or type into the delay input) to set that node's `motion.delay`; **Auto-stagger** distributes reveal delays evenly (120ms steps in tree order); **Play all** dispatches the preview for every node. All edits write `motion.delay` through the already-audited `editor.updateProps` → bake path (server bounds delay 0–5000). Delivers the *value* of Mosaic's After-Effects timeline (visual staggering/sequencing) without a full multi-track keyframe editor, per the borrow doc's "don't chase timeline depth" guidance.

**The Mosaic borrow list is now complete** — every item (#1–#17) is shipped or intentionally superseded.
...
- **All-screens canvas (Figma-style)** — Desktop / Tablet / Mobile rendered side by side as **full-height pages** on an infinite canvas: zoom (⌘/Ctrl-scroll, − / + / Fit), pan (scroll or drag), square edges, dotted grid. Each frame shows the real responsive layout live (media queries + custom-media breakpoints apply per width). No per-frame scrolling.
- **Zoom into one** — click a frame's label to jump into that single device.
- **Reusable frame** — `CanvasFrame.svelte` extracts the iframe canvas (with full-height measurement) so any number of frames render the same node tree + global CSS simultaneously, all selection-bridged to the editor.
