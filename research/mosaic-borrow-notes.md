...
**Status:** Research / observation. **Shipped so far:** #1 theme-first picker + #2 animated import (mosaic-wizard); **#11 loop element + visual dynamic-data** (v6.0.0-beta.36, bakes to `<outpost-each>`, verified live); **#3 numbered orientation** + **#4 "Made it yours" recap** (v6.0.0-beta.36); **#9 section/pattern gallery** (v6.0.0-beta.37, live-thumbnail picker → `importSection`) + **#7 preview toggle + View live** (v6.0.0-beta.37); **#16 style-guide editor** (v6.0.0-beta.38, tokens now live-apply to the site); **#10 templates archive** (v6.0.0-beta.39, grid → Code Editor). Editor-shell items #5/#6/#15/#17 landed via the v6 builder arc. **Remaining:** only the interaction cluster — #8 slash-insert, #12 scroll/click interactions, #13 After-Effects timeline, #14 select-targets-on-canvas.
**Date:** 2026-07-03
